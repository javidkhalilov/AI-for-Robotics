# -*- coding: utf-8 -*-
"""
Created on Mon Jul 25 21:24:22 2016
...
def localize(colors,measurements,motions,sensor_right,p_move):
    ...
    q=[[] for row in range(len(colors))]
    hit=0
    
    for i in range(len(colors)):
        #print(colors[i],len(colors[i]))
        for j in range(len(colors[i])):
            #print(colors[i][j])
            hit=(measurements[j]==colors[i][j])
            q[i].append(p[i][j]*(hit*sensor_right+(1-hit)*(1.0-sensor_right)))
#            print(q[i][j])
            p[i][j]=q[i][j]
#            print(p[i][j])
